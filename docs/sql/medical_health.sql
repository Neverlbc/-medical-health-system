...
CREATE TABLE `patient_allergy` (
  ...
  `allergy_type` VARCHAR(20) NOT NULL COMMENT '类型(DRUG:药物, FOOD:食物, ENV:环境, OTHER:其他)',
...
-- ====================================================
-- 18. 诊断记录表 (diagnosis_record)
-- ====================================================
DROP TABLE IF EXISTS `diagnosis_record`;
CREATE TABLE `diagnosis_record` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '诊断记录ID',
  `patient_id` BIGINT NOT NULL COMMENT '患者ID',
  `diagnosis_type` VARCHAR(20) NOT NULL COMMENT '类型(PRELIMINARY:初步诊断, CONFIRMED:确诊)',
...
CREATE TABLE `lab_test` (
  ...
  `report_url` VARCHAR(500) DEFAULT NULL COMMENT '报告单图片/PDF链接',
...
CREATE TABLE `follow_up` (
  ...
  `method` VARCHAR(20) NOT NULL COMMENT '方式(OUTPATIENT:门诊, PHONE:电话, HOME:家访)',
